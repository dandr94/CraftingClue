...
function CraftingClue:InitializeUI()
    ...
    self.reagentText = ClassTrainerFrame:CreateFontString(nil, "OVERLAY", CraftingClue.Config.UI.FONT)
...
function CraftingClue:GetItemName(itemId)
    ...
    if not itemName then
        -- This is a common technique to force the client to cache item data.
        GameTooltip:SetOwner(UIParent, "ANCHOR_NONE")
        GameTooltip:SetHyperlink("item:" .. itemId)
        GameTooltip:Hide()
        itemName = GetItemInfo(itemId) -- Try again now that it should be cached.
    end
...
-- @param text (string): The formatted text to display if any, otherwise hide the element.
function CraftingClue:DisplayReagentText(text)
    self:InitializeUI() -- Ensure the frame exists
    self.reagentText:SetText(text or "")
...
function CraftingClue:UpdateReagentText(recipeName)
    -- If the trainer isn't a tradeskill trainer or isn't visible, clear the text.
    -- For some reason the close event doesn't clears the text.
    if not IsTradeskillTrainer() or not ClassTrainerFrame:IsVisible() then
    ...
    if not craft or not craft.reagent_id_to_count then
        self:DisplayReagentText(self.Config.TEXT.REAGENT_TITLE .. "|c" .. self.Config.UI.COLORS.ERROR .. self.Config.TEXT.NOT_APPLICABLE .. "|r")
        return
    end
    ...
    self:DisplayReagentText(fullText)
end
